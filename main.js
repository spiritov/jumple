...
function initializeSwapDayElements() {
  if (document.getElementById('index_identifier') != null || day == currentDay) {
    //latest page
    initializeLeftLink();
  } else if (day == 1) {
    initializeRightLink();
...
function initializeAutofill() {
  inputElement.addEventListener('keydown', function (pressed) {
...
function show(hint_number) {
  if (hint_number <= max_guesses) {
    //don't try to show a hint that doesn't exist
...
function checkIfSolved(solved) {
  ...
  } else //incorrect guess
  {
    ...
    if (guesses < max_guesses) {
      ...
      setTimeout(() =>
        //wait for hint button animation to finish
        {
          show(guesses + 1);
        }, 500);
...
function shareResults() {
  ...
  clipboard.onclick = function () {
    navigator.clipboard.writeText('🚀 Jumple Day ' + day + '\n' + resultsString);
...
function checkLocalStorage() {
  if (
    localStorage.getItem('day' + day + '_result') &&
    localStorage.getItem('day' + day + '_history')
  ) {
    //populate game state
